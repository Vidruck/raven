...
#[pyfunction]
fn parse_sync_state<'py>(py: Python<'py>, payload_json: &str) -> PyResult<(Bound<'py, PyDict>, Bound<'py, PyList>)> {
    
 
    let payload: KWinPayload = match serde_json::from_str(payload_json) {
        Ok(p) => p,
        Err(e) => return Err(pyo3::exceptions::PyValueError::new_err(format!("Error de Serde: {}", e))),
    };
